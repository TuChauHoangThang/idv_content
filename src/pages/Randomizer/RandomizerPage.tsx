...
const RandomizerPage: React.FC = () => {
  ...
  const [missionWinner, setMissionWinner] = useState<string | null>(null);
  ...
  // Added state to force re-render for UI updates if needed, though button disabled states rely on it.
  ...
  const rollSurvivors = () => {
    ...
    setBranchResults([]); // Reset branch results when rolling new survivors
    setMissionWinner(null); // Reset mission winner
    confetti({
      particleCount: 50,
      spread: 60,
      origin: { y: 0.8 },
  ...
  const handleBranchWin = (winner: WheelItem) => {
    setBranchResults(prev => {
      ...
      // If we are auto-spinning and haven't reached 4 yet, trigger next spin
      if (autoSpinCount.current > 0 && newResults.length < 4) {
        setTimeout(() => {
          branchWheelRef.current?.spin(2000); // 2s duration for fast consecutive spins
        }, 500); // small delay between spins
      } else if (newResults.length === 4) {
        autoSpinCount.current = 0;
        setIsBranchSpinning(false);
        
        // Small confetti pop for finishing all 4 branches
        confetti({
          particleCount: 100,
          spread: 70,
          origin: { y: 0.6 },
        ...
        // If part of mega roll, now spin the mission wheel
        if (isMegaRolling.current) {
          setTimeout(() => {
            missionWheelRef.current?.spin();
  ...
  const handleMissionWin = (winner: WheelItem) => {
    ...
    setTimeout(() => setShowMissionBanner(false), 4000);
  };
  ...
  const triggerBranchAutoSpin = () => {
    ...
    branchWheelRef.current?.spin(2000); // start the first spin (fast)
  };
  ...
  const handleMegaRoll = () => {
    ...
    // Step 1: Roll Survivors
    setSelectedSurvivors(getRandomItems(survivors, 4));
    setBranchResults([]);
    setMissionWinner(null);
    
    // Step 2: Trigger Branch Auto Spin after a short delay
    setTimeout(() => {
      autoSpinCount.current = 4;
  ...
  const renderCharacterCard = (char: Character | null, index: number) => {
    ...
    return (
      ...
      >
        ...
        {/* Branch result assigned to this player */}
        <AnimatePresence>
          {branch && (
            <motion.div 
        ...
        <button 
          ...
          onClick={() => refreshSurvivor(index)}
          title="Refresh this character"
        >
          <RefreshCw size={18} />
        </button>
      </motion.div>
  ...
  return (
    <div className="randomizer-container">
      ...
      <div className="mega-roll-container">
        ...
        <p className="mega-desc">Tự động chọn 4 Survivor + 4 Nhánh kỹ năng + 1 Nhiệm vụ chung</p>
      </div>

      {/* Survivor Randomizer */}
      <div className="section survivors-section">
        <div className="section-header">
          <h2><Users size={24} /> Survivors (4 Players)</h2>
        ...
        <motion.div className="cards-grid" layout>
          <AnimatePresence mode='popLayout'>
            {selectedSurvivors.length > 0 
              ...
              : Array(4).fill(null).map((_, i) => renderCharacterCard(null, i))}
          </AnimatePresence>
        </motion.div>

        {/* Team Mission Display */}
        <AnimatePresence>
          {missionWinner && (
            <motion.div 
            ...
            >
              <h3><Sparkles size={20} /> Nhiệm vụ Team</h3>
              <p className="mission-text">{missionWinner}</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="wheels-section">
        ...
        <div className="section wheel-card">
          <div className="section-header">
            <h2><Target size={24} /> Vòng Quay Nhánh</h2>
          </div>
          <p className="wheel-description">
            Random nhánh kỹ năng cho 4 Survivor!
          </p>
          ...
          <div className="mission-editor" style={{ marginTop: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn-icon" onClick={shuffleBranchText} title="Trộn ngẫu nhiên" style={{ background: 'rgba(255,255,255,0.1)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem' }}>🔀 Trộn</button>
                <button className="btn-icon" onClick={() => setBranchText('')} title="Xóa tất cả" style={{ background: 'rgba(255,255,255,0.1)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem' }}>🗑️ Xóa hết</button>
              </div>
            </div>
            <textarea
              className="input-field mission-textarea"
              value={branchText}
              onChange={(e) => setBranchText(e.target.value)}
              placeholder="Nhập nhánh, mỗi dòng 1 nhánh..."
              rows={6}
            />
          </div>
        </div>
        ...
        <div className="section wheel-card">
          <div className="section-header">
            <h2><Sparkles size={24} /> Vòng Quay Nhiệm Vụ</h2>
          </div>
          <p className="wheel-description">
            Thêm nhiệm vụ vui nhộn để thực hiện trong trận! 🎉
          </p>

          <AnimatePresence>
            {showMissionBanner && missionWinner && (
              <motion.div 
          ...
          <div className="mission-editor">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn-icon" onClick={shuffleMissionText} title="Trộn ngẫu nhiên" style={{ background: 'rgba(255,255,255,0.1)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem' }}>🔀 Trộn</button>
                <button className="btn-icon" onClick={() => setMissionText('')} title="Xóa tất cả" style={{ background: 'rgba(255,255,255,0.1)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem' }}>🗑️ Xóa hết</button>
              </div>
            </div>
            <textarea
              className="input-field mission-textarea"
              ...
              placeholder="Nhập nhiệm vụ, mỗi dòng 1 nhiệm vụ..."
              rows={6}
            />
          </div>
        </div>
